...
const getUserAllTasks = async (request, reply) => {
  try {
    ...
    const allTasks = await Task.aggregate([
      ...
      // Unwind the handyman array to get individual handyman documents
      { $unwind: { path: '$handyman', preserveNullAndEmptyArrays: true } },
      // Lookup user information to get handyman's name
      {
        $lookup: {
          from: 'users',
      ...
      // Unwind the handymanUser array to get individual user documents
      { $unwind: { path: '$handymanUser', preserveNullAndEmptyArrays: true } },
      // Project fields to include in the result
      {
        $project: {
          _id: 0, // Exclude the MongoDB _id field
          ...
          taskStatus: 1,
          handyman_name: {
            $concat: ['$handymanUser.firstName', ' ', '$handymanUser.lastName'],
          },
          handyman_number: '$handymanUser.phoneNumber',
        },
      },
    ]);
